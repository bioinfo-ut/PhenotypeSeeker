...
class phenotypes():
    ...
    def LR_feature_selection(self):
        ...
        probs_base = model.predict_proba(self.PCA_df[[f"PC_{i+1}" for i in range(self.nr_pcs)]])
        ...
        for kmer in LRdf:
            alt_df = pd.merge(self.PCA_df[[f"PC_{i+1}" for i in range(self.nr_pcs)]], LRdf[kmer], left_index=True, right_index=True)
    ...
    def write_model_coefficients_to_file(self):
        ...
        if self.model_name_short == "linreg":
            ...
            feat_importances = self.model_fitted.best_estimator_.feature_importances_
            coefs = pd.Series(
                feat_importances,
                index=self.ML_df.columns[:len(feat_importances)]
                )
        elif self.model_name_short == "log_reg":
            feat_importances = self.model_fitted.best_estimator_.coef_[0]
            coefs = pd.Series(
                feat_importances,
                index=self.ML_df.columns[:len(feat_importances)]
                )
